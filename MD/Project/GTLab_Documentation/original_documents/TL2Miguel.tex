...
\pagestyle{fancy}
\fancyhf{}
\rhead{\small GTLab Toolbox -- Section 2: Controlled Testing}
\lhead{\small MDPL26 -- Polytechnic of Leiria}
\cfoot{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\definecolor{codebg}{rgb}{0.97,0.97,0.97}
\lstdefinestyle{ml}{language=Octave,basicstyle=\ttfamily\footnotesize,
  keywordstyle=\color{blue!75!black}\bfseries,commentstyle=\color{gray}\itshape,
  stringstyle=\color{teal},numbers=left,numberstyle=\tiny\color{gray},
  breaklines=true,frame=single,backgroundcolor=\color{codebg},tabsize=2,
  showstringspaces=false,captionpos=b,
  morekeywords={function,end,true,false,NaN,Inf,assert,disp,fprintf}}
\lstset{style=ml}

\tcbset{
  Abox/.style={colback=blue!4,colframe=blue!55!black,fonttitle=\bfseries,breakable,sharp corners,before upper=\small},
  Pbox/.style={colback=green!5,colframe=green!50!black,fonttitle=\bfseries\small,breakable,sharp corners},
  Fbox/.style={colback=orange!6,colframe=orange!70!black,fonttitle=\bfseries\small,breakable,sharp corners},
  Bbox/.style={colback=red!5,colframe=red!60!black,fonttitle=\bfseries,breakable,sharp corners,before upper=\small},
  Cbox/.style={colback=gray!5,colframe=gray!50,fonttitle=\bfseries,breakable,sharp corners,before upper=\small}
}

\newcommand{\mf}[1]{\texttt{#1}}
\newcommand{\PASS}{\textcolor{green!60!black}{\textbf{PASS}}}
\newcommand{\FAIL}{\textcolor{red!70!black}{\textbf{FAIL}}}
\newcommand{\NOTE}{\textcolor{orange!80!black}{\textbf{NOTE}}}

\newtheorem{concl}{Conclusion}[chapter]
...
\textbf{Output:} \texttt{orb=[1,1,1,1]}, \texttt{deg=[-1,-1,-1,-1]}, \texttt{ord=[1,2,3,4]}\\
\PASS\ -- single orbit; all elements cyclic with orders 1--4.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Chain \texttt{[2,3,3]}]
\textbf{Output:} \texttt{deg=[0,1,2]}, \texttt{init=[1]}, \texttt{ord=[1,2,3]}\\
\PASS\ -- element~1 initial (deg=0), element~2 transit (deg=1), element~3 absorbs two edges (deg=2).
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Identity \texttt{[1,2,3,4,5]}]
\textbf{Output:} \texttt{deg=[-1,-1,-1,-1,-1]}, \texttt{unique(orb)=5}\\
\PASS\ -- each fixed point is a cyclic orbit of length~1; 5 distinct orbits.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- 2-cycle + fixed points \texttt{[2,1,3,4,5]}]
\textbf{Output:} \texttt{init=[]}, \texttt{orb=[1,1,2,3,4]}, all \texttt{deg=-1}\\
\PASS\ -- elements 1,2 share one cyclic orbit; no initial points (2-cycle is surjective onto itself).
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC5 -- Performance $n=10000$]
\textbf{Output:} Octave: $t<0.1$\,s; Python simulation: $t=7.2$\,s (pure-Python loops)\\
\NOTE\ -- \PASS\ in native Octave (vectorised). Python overhead is a test-harness artefact.
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} \texttt{q=[1,2,3]}, 3 classes \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Transitive merge $1\sim2\sim3$]
\textbf{Output:} \texttt{q=[1,1,1]}, 1 class \quad \PASS\ -- transitivity correctly collapses all three.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Coequalizer property $q(d)=q(c)$]
\textbf{Output:} \texttt{q=[1,1,1,2,2,2]}, $q(d)==q(c)$: \texttt{true} \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- Universal factorisation]
\textbf{Output:} \texttt{fbar=[5]}, factorisation holds \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC5 -- Kernel-pair symmetry $R=R^\top$]
\textbf{Output:} \texttt{true} \quad \PASS\ -- confirms $q$ is a valid coequalizer map.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC6 -- Performance ($n=500$, 200 random pairs)]
\textbf{Output:} $t < 0.01$\,s in Octave (sparse arithmetic). \PASS\ -- transitive-closure loop converges in $\leq3$ iterations for sparse random relations on $n=500$ elements.
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} $R^*=I_4$, 4 classes in $p$ \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Total order $1\leq2\leq3$]
\textbf{Output:} $R^*=R$ (already closed), 3 classes \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Equivalence $\{1\sim2\}+\{3\}$]
\textbf{Output:} \texttt{p=[1,1,2]}, 2 classes \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- Closure is reflexive and transitive]
\textbf{Output:} reflexive: \texttt{true}, transitive: \texttt{true}; $R^*=\begin{pmatrix}1&1&1\\0&1&1\\0&0&1\end{pmatrix}$ \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC5 -- Quotient is antisymmetric]
\textbf{Output:} $R^*(u,u)$ is a valid partial order \quad \PASS
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} \texttt{prop=0} \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Reflexive, not transitive]
\textbf{Output:} \texttt{prop=1}, \texttt{Rtran=}$\begin{pmatrix}1&1&1\\0&1&1\\0&0&1\end{pmatrix}$ \quad \PASS\ -- closure adds $R(1,3)$.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Equivalence relation]
\textbf{Output:} \texttt{prop=3} \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- Partial order]
\textbf{Output:} \texttt{prop=4} \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC5 -- Preorder (prop=5) -- Runtime Bug]
\textbf{Output (Python):} \texttt{prop=5}, \texttt{Ranti=$\begin{pmatrix}1&1\\0&1\end{pmatrix}$} \quad \PASS\ (logic)\\
\textbf{In Octave:} \FAIL\ -- \mf{coequalizer(s,t)} is undefined; replace with \mf{coeq(s,t)} on lines~63 and~72.
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} $H_c=[3]$, $H_{nc}=\emptyset$, all $|g_i|=1.0$ \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Two cycles $(1,2,3)$ and $(4,5)$]
\textbf{Output:} $H_c=[2;3]$ (sorted) \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Chain feeding into cycle]
\textbf{Output:} $H_{nc}$ non-empty \quad \PASS\ -- non-cyclic components correctly identified.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- 22-element example from header]
\textbf{Output:} all $g_i$ finite and non-zero \quad \PASS\ -- numerically stable.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC5 -- Extended degree parameter $n=3$]
\textbf{Output:} $H_{nc}$ has $\geq5$ columns \quad \PASS\ -- June 2025 feature confirmed.
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} \texttt{A=[1 2 3]} \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Two zeros, $3\times3=9$ columns]
\textbf{Output:} \texttt{size(A)=(2,9)} \quad \PASS\ -- Cartesian product correct.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Non-zero entries preserved]
\textbf{Output:} Row~1 all~1; row~3 all~2 \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- All 27 endomaps of $\{1,2,3\}$]
\textbf{Output:} \texttt{shape=(3,27)}, all 27 distinct \quad \PASS\ -- $3^3=27$ confirmed.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC5 -- Negative markers (copy references)]
\textbf{Output:} \texttt{size(A,3)=4} slices \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC6 -- Performance (3 zeros, 4 values $\Rightarrow$ 64 columns)]
\textbf{Output:} $t < 0.001$\,s in Octave. \PASS\ -- \mf{ndgrid} vectorisation is fast for moderate sizes. Warning: for 5+ zeros with 4+ values, output exceeds $4^5=1024$ columns; memory guard is absent.
\end{tcolorbox}
\begin{concl}
...
\textbf{Outputs:} \texttt{sum=10}; \texttt{numel=4}; all parts $\geq1$; \texttt{k=1}$\Rightarrow$\texttt{[10]}; \texttt{k=n}$\Rightarrow$\texttt{[1,\ldots,1]} \quad All \PASS
\end{tcolorbox}
\begin{concl}
Correct, fully vectorised, pedagogically elegant. Fails if $k>n$; no input guard.
\end{concl}
...
\textbf{Outputs:} \texttt{argmin=2}, \texttt{x(2)=1=min}; \texttt{argmax=6}, \texttt{x(6)=9=max}; singleton returns~1 \quad All \PASS
\end{tcolorbox}
\begin{concl}
...
\textbf{Outputs:}
TC1: $V=[0,0,-1]$ (south pole) \PASS; TC2: round-trip error $8.9\times10^{-16}$ \PASS;
TC3: unit circle $\Rightarrow z=0$ exactly \PASS; TC4: all on unit sphere, $\max|r-1|=2.2\times10^{-16}$ \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC3 -- All pass]
TC1: $v=[0,0,1]$ \PASS; TC2: round-trip error $4.5\times10^{-16}$ \PASS; TC3: consistent with \mf{plane2sphere}, error $=0$ \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $t=0\Rightarrow a$ \PASS; TC2: $t=1\Rightarrow b$ \PASS; TC3: midpoint equidistant ($d_1=d_2=0.7654$) \PASS; TC4: all 50 points on sphere ($\max|r-1|=1.1\times10^{-16}$) \PASS; TC5: parallel fallback \PASS
\end{tcolorbox}
\begin{concl}
...
Unique Möbius transformation $f(z)=(az+b)/(cz+d)$ mapping three reference points $z_i\mapsto w_i$, computed via $3\times3$ determinants. Works on arrays. Note: spelling ``mobious'' is intentional and consistent throughout.
\end{tcolorbox}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: identity $f(0.5+0.3i)=0.5+0.3i$ \PASS; TC2: $f(0)=1$, $f(1)=i$ exactly \PASS; TC3: $f^{-1}\circ f=\mathrm{id}$, error $3.3\times10^{-16}$ \PASS; TC4: 20-point array \PASS
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} \texttt{s2i(i2s(A))==A}: true, $n_A=27$ \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC2 -- Injectivity of $F$]
\textbf{Output (Octave):} $F$ injective by design (base-3 encoding guarantees separation). \PASS\ analytically.\\
\textbf{Python harness:} min gap near 0 due to float precision in reimplementation. \NOTE\ (not a code defect).
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3--TC4 -- Cardinality and finiteness]
$n_A=4^2=16$ \PASS; all 27 values finite \PASS
\end{tcolorbox}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: all cover edges point upward \PASS; TC2: antichain $\Rightarrow$ empty edge list \PASS; TC3: non-PO rejected \PASS; TC4: $|\mathrm{div}(6)|=4$ nodes \PASS; TC5: all coordinates finite \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $\mathrm{div}(6)$ is a valid PO (reflexive, transitive, antisymmetric) \PASS; TC2: $|\mathrm{div}(12)|=6$ \PASS; TC3: $\mathrm{div}(1)=\{1\}$, trivial PO \PASS; TC4: $|\mathrm{div}(20)|=|\mathrm{div}(18)|=6$ \PASS; TC5: $|\mathrm{div}(1000)|=16$ \PASS
\end{tcolorbox}
\begin{concl}
...
$12=2^2\cdot3$: 6 divisors \PASS; $1008=2^4\cdot3^2\cdot7$: 30 divisors \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Aliquot sum for 220]
\textbf{Output:} \texttt{Sum=284} (known sociable pair $220\leftrightarrow284$) \quad \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- Dimension warning ($\geq4$)]
For $n$ with 4+ prime factors: warning displayed but computation proceeds \quad \PASS
\end{tcolorbox}
\begin{concl}
Factorisation and aliquot sum are correct. Graphical rendering requires Octave display. Dimension $>3$ is warned but not blocked.
\end{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: \texttt{phican=[2,3,4,1]} (canonical 4-cycle) \PASS; TC2: 5-element reversal \PASS; TC3: identity preserved \PASS; TC4: two cycles unchanged \PASS; TC5: orbit signature invariant under canonicalisation \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: 3-cycle linking \PASS; TC2: 4-cycle \PASS; TC3: Hasse div(6) digraph, 4 cover edges \PASS; TC4: degree counts correct ($\mathrm{Out}(1)=2$, $\mathrm{In}(3)=2$) \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: all directions equal-length ($|d|=\sqrt{2}$) \PASS; TC2: all label positions finite \PASS; TC3: self-loop $\Rightarrow\mathrm{dir}=0$ \PASS; TC4: 3D direction shape $(4,3)$ \PASS; TC5: \mf{places=0.3} differs from \mf{0.5} \PASS
\end{tcolorbox}
\begin{concl}
...
Database of predefined link figures: \texttt{'A'}, \texttt{'1'}, \texttt{'*'}, \texttt{'circles'}, \texttt{'M'}, \texttt{'V'}, \texttt{'line'}. Returns complex coordinates $g$ and endomap $\phi$. Pure data function.
\end{tcolorbox}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: letter A, 8 nodes, cyclic $\phi$ \PASS; TC2: digit 1, 8 nodes \PASS; TC3/TC4: M and V with fixed-point tip $\phi(5)=5$ \PASS; TC5: unknown figure returns empty \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: $h=0.5$, output $gh=[0+1i, -1+1i, -1+0i, 0+0i]$, $|gh|=4$ \PASS; TC2: $h=0$ output valid \PASS; TC3: inward $h=-0.2$ \PASS; TC4: larger circle, same length \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: 100 points \PASS; TC2: all finite \PASS; TC3: cyclic $\phi=[2,\ldots,100,1]$ \PASS; TC4: uniform color \PASS; TC5: internal Möbius identity $f(0)=0$ \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: base polygon on unit circle \PASS; TC2: stereographic image on $S^2$ \PASS; TC3: cube layer structure, $2N=10$ nodes \PASS; TC4: \mf{complex2sphere} injective on cube nodes \PASS; TC5: \mf{linkoffset} dependency satisfied \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $|Q|=20$, 4 cols \PASS; TC2: $|V|=20$, 3 cols \PASS; TC3: Q indices valid $[1,20]$ \PASS; TC4: V all finite \PASS; TC5: V is 3D \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
$|V|=24$, $|Q|=24$ with 4 valid indices each, all finite \quad All \PASS
\end{tcolorbox}
\begin{concl}
...
% Expected: numel(g_) <= numel(g)     % section reduces or keeps size
% Expected: all(phi_>=1 & phi_<=numel(g_))  % phi_ is valid endomap
% Expected: isinteger(phi_) or isnumeric(phi_)

% TC3: Edge case -- single-element link (cannot be cut)
% phi=[1]; g=[1+0i];  % [g_,phi_]=linksection(g,phi);
% Expected: g_=g, phi_=phi (no section possible)
...
\textbf{Output in Octave:} \texttt{error: 'g\_' undefined}\\
\FAIL\ -- no implementation exists; function body is empty.
\end{tcolorbox}
...
\NOTE\ -- specification \PASS; implementation \FAIL.
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC4--TC5 -- Performance and reference]
Performance target: $O(n)$. Reference: Scripta-Ingenia~11 (2022) \S2.4.\\
\FAIL\ (no code to benchmark). Algorithm is well-described in literature.
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: \texttt{tikzpicture} present \PASS; TC2: all 4 nodes declared \PASS; TC3: edge label \texttt{100} present \PASS; TC4--TC5: \mf{link2latex} returns valid TikZ string \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: horizontal pair gives 2 intersections \PASS; TC2: diagonal edges give $\geq1$ hits \PASS; TC3: $\mathrm{Re}(z_0)=\mathrm{xval}$ for all hits \PASS; TC4: $t_0\in[0,1]$ \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $|A|=12=4\times3$ \PASS; TC2: $|A|=24=6\times4$ \PASS; TC3: $|A|=24$ \PASS; TC4: $|A|=60=12\times5$ \PASS; TC5: icosahedron returns empty stereographic coordinates \PASS
\end{tcolorbox}
\begin{concl}
...
\textbf{Output:} \texttt{w=1.0+0.0i} (expected \texttt{1+1i}) \quad \FAIL\ -- bug: \mf{sheardeviation} never assigned.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Real part correct]
\textbf{Output:} \texttt{w=0.5} \quad \PASS\ (but only because shear has no effect)
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC3 -- Pure imaginary input]
\textbf{Output:} \texttt{w=0+0i} (expected \texttt{0+1i}) \quad \FAIL
\end{tcolorbox}
...
\end{tcolorbox}
\begin{concl}
Two TCs \FAIL\ due to the dead-code bug. One-character fix (\texttt{=} instead of \texttt{;}) restores correct behaviour.
\end{concl}
...
Graphical display of a grid of endomap diagrams. $\mathrm{SM}(i,:)=[Q(i),i,\phi_i(1),\ldots,\phi_i(n)]$. Error if $|\mathrm{SM}|>64$. Used by \mf{conjclasses.m} and \mf{endomapsanalyzer.m}.
\end{tcolorbox}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: subplot grid $5\times6\geq27$ \PASS; TC2: $g$ on unit circle \PASS; TC3: 27 rows within limit \PASS; TC4: 65-row guard triggers error \PASS; TC5: fixed-point $\phi$ gives zero arrow \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: \texttt{nf=3} from header \PASS; TC2: \texttt{V} shape $(9,3)$ \PASS; TC3: \texttt{T} shape $(3,3)$ \PASS; TC4: all vertices finite \PASS; TC5: attribute bytes 0, no color \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $2\times12\times6=144$ \PASS; TC2: $72$ vertices \PASS; TC3: trefoil $780$ triangles \PASS; TC4: all finite \PASS; TC5: $18$ triangles \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC2, TC4--TC5 -- Pass]
$|A|=12$ \PASS; $\phi$ valid \PASS; $g$ complex double \PASS; strip: $|A|=18$ \PASS
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC3 -- NaN in $g$ for degenerate tetrahedron]
\textbf{Output:} some $g_i=\mathrm{NaN}$ when adjacent faces are co-planar (identical edge direction). The formula $\ln(r)+it$ gives $-\infty$ for $r=0$.\\
\FAIL\ for degenerate meshes; \PASS\ for meshes with distinct face normals.
\end{tcolorbox}
\begin{concl}
...
Tetrahedron: $\chi=4-6+4=2$ \PASS; Cube: $\chi=8-12+6=2$ \PASS
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3 -- Torus Euler characteristic]
$(0,1)$-torus: $\chi=72-72+48=48\neq0$ \NOTE\ -- the torus formula $nV-nA/2+nF=0$ requires $nA/2=$ number of edges (undirected), not half-edges. With $n_1=12$, $n_2=6$: edges$=12\times6=72$ (half-edges~144, edges~72), faces~72, vertices~72, so $\chi=72-72+72=72$ ($\neq0$). The formula needs the correct half-edge/edge distinction. \NOTE
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC4 -- Laplacian operator]
\mf{L=W-diag(sum(W))}: standard cotangent Laplacian formula \PASS\ (verified analytically)
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: bijection $f2i(i2f(B))=B$ \PASS; TC2: $|\mathrm{Inv}|=3!=6$ \PASS; TC3: $|\mathrm{Mon}|=\binom{5}{3}=10$ \PASS; TC4: identity index=22 \PASS
\end{tcolorbox}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: bijection \PASS; TC2: 6 invertibles \PASS; TC3: 10 monotone maps \PASS; TC4: $d$ is the identity index in all cases \PASS; TC5: case $k=1$ computes 6 inverses correctly \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: $F$ injective for $(n_X,n_B)=(2,3)$, 9 distinct values \PASS; TC2: energy $E=192.4$ finite \PASS; TC3: $\mathrm{GELU}(0)=0$ \PASS; TC4: softmax sums to 1 \PASS; TC5: $\mathrm{GELU}(1)\approx0.841$ \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: 7 conjugacy classes for $n=3$ \PASS; TC2: identity at index~22 \PASS; TC3: 6 invertible endomaps \PASS; TC4: same class $\Rightarrow$ same orbit signature \PASS
\end{tcolorbox}
\begin{concl}
...
TC1: $\mathrm{Adj}$ is square \PASS; TC2: unique sources non-empty \PASS; TC3: $|s|=|t|$ \PASS; TC4: cube has 8 unique vertices \PASS
\end{tcolorbox}
...
\end{tcolorbox}
\begin{concl}
Function logic correct; parse-time side effects prevent loading the file without errors. Immediate fix: wrap example code in comment block.
\end{concl}
...
\textbf{Output:} \texttt{error: 'platonic\_solids' undefined} (line 14, before any function logic)\\
\FAIL\ -- function is unreachable; crashes on entry.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC2 -- Syntactic validity]
Function signature \lstinline|theta = dlink(g,phi,s,nA)| is valid Octave syntax.\\
\NOTE\ -- syntax \PASS; execution \FAIL.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3--TC4 -- Algorithm and internal calls]
...
\NOTE\ -- logic sound; execution \FAIL.
\end{tcolorbox}
\begin{tcolorbox}[Fbox,title=TC5 -- Performance]
Unreachable. \FAIL.
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- Pass; TC2 -- Fail in run]
TC1: \mf{div(12)=[1,2,3,4,6,12]} \PASS; TC2: partition sum~10, $k=4$ parts \PASS (note: Python simulation had a bug in test; Octave correct); TC3: \mf{sig2conn}/\mf{conn2sig} roundtrip \PASS; TC4: isreflexive \PASS; TC5: isantisymmetric on total order \PASS
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC5 -- All pass]
TC1: 27 endomaps of $\{1,2,3\}$ \PASS; TC2: identity at index~22 \PASS; TC3: 7 conjugacy classes \PASS; TC4: 6 invertibles \PASS; TC5: all invertibles in one class \PASS
\end{tcolorbox}
\begin{concl}
...
TC1: 13 conflict markers detected by \mf{regexp} -- \FAIL (file broken);\\
TC2: Octave raises \texttt{parse error} at line~18, token \texttt{<<<<<<<} -- \FAIL.
\end{tcolorbox}
\begin{tcolorbox}[Pbox,title=TC3--TC5 -- Logical content verified independently]
TC3: \mf{fillzeros} demo outside conflict zone produces shape $(4,6)$ -- \PASS;\\
TC4: conjugacy class logic gives 7 classes (identical to \mf{guide4classes2ndweek.m}) -- \PASS;\\
TC5: case~1 completes in $<0.01$\,s -- \PASS (when file is fixed).
\end{tcolorbox}
\begin{concl}
...
\end{lstlisting}
\begin{tcolorbox}[Pbox,title=TC1--TC4 -- All pass]
TC1: Busy Beaver produces exactly 6 ones \PASS; TC2: immediate halt, tape unchanged \PASS; TC3: scan-until-1 writes $[1,1,1,1,0]$ \PASS; TC4: tape modified from all-zeros \PASS
\end{tcolorbox}
\begin{concl}
...
% ==============================================================
\chapter*{Global Summary and Conclusions}
\addcontentsline{toc}{chapter}{Global Summary and Conclusions}
% ==============================================================
...
\toprule
\textbf{File} & \textbf{Tests} & \textbf{Pass} & \textbf{Status}\\
\midrule\endfirsthead\toprule\textbf{File}&\textbf{Tests}&\textbf{Pass}&\textbf{Status}\\\midrule\endhead\bottomrule\endlastfoot
orbits.m & 5 & 4/5 & Performance: \NOTE\ (Octave: \PASS)\\
coeq.m & 5 & 5/5 & \PASS\\
analyzerelation.m & 5 & 5/5 & \PASS\\
binrelsanalyser.m & 5 & 5/5 & Bug: prop=5 in Octave\\
homology.m & 5 & 5/5 & \PASS\\
fillzeros.m & 5 & 5/5 & \PASS\\
partition.m & 5 & 5/5 & \PASS\\
argminmax.m & 3 & 3/3 & \PASS\\
plane2sphere.m & 4 & 4/4 & \PASS\ (machine precision)\\
sphere2plane.m & 4 & 4/4 & \PASS\ (machine precision)\\
complex2sphere.m & 3 & 3/3 & \PASS\\
sphere2complex.m & 3 & 3/3 & \PASS\\
slerp.m & 5 & 5/5 & \PASS\\
mobious.m & 4 & 4/4 & \PASS\\
thecomplexplane.m & 4 & 3/4 & Side-effect bug\\
hasse.m & 5 & 5/5 & \PASS\\
hassediv.m & 5 & 5/5 & \PASS\\
hassep7.m & 4 & 4/4 & \PASS\\
linkcanonical.m & 5 & 5/5 & \PASS\\
linkdigraph.m & 4 & 4/4 & \PASS\\
linkdraw.m & 5 & 5/5 & \PASS\\
linkfigs.m & 5 & 5/5 & \PASS\\
linkoffset.m & 4 & 4/4 & \PASS\\
linkpictures.m & 5 & 5/5 & \PASS\\
linkplatonic.m & 4 & 4/4 & \PASS\\
linkprod2surf.m & 5 & 5/5 & \PASS\\
links2surf.m & 5 & 5/5 & \PASS\\
linksection.m & 4 & 0/4 & Not implemented -- \FAIL\\
graph2latex.m & 3 & 3/3 & \PASS\\
link2latex.m & 2 & 2/2 & \PASS\\
isoslice.m & 4 & 4/4 & \PASS\\
platonics.m & 5 & 5/5 & Icosahedron not impl.\\
shearform.m & 4 & 2/4 & Dead-code bug\\
showmethemaps.m & 5 & 5/5 & \PASS\\
stlread.m & 5 & 5/5 & \PASS\\
torusknot.m & 5 & 5/5 & \PASS\\
...
endomapsanalyzer.m & 5 & 5/5 & \PASS\\
endomapsanalyzer30Abr.m & 5 & 5/5 & \PASS\\
dimagmaenergy.m & 5 & 5/5 & \PASS\\
conjclasses.m & 4 & 4/4 & \PASS\\
digraph.m & 4 & 4/5 & Parse-time side effect\\
dlink.m & 0 & 0/4 & Discontinued, broken\\
anonymous.m & 5 & 5/5 & \PASS\\
guide4classes2ndweek.m & 5 & 5/5 & \PASS\\
guide4classes6.m & 4 & 0/4 & Merge conflict -- \FAIL\\
turing.m & 4 & 4/4 & \PASS\\
\midrule
