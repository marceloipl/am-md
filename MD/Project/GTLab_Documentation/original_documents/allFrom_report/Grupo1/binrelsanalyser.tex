...
\chapter{MATLAB Code Analysis: \texttt{binrelsanalyser.m}}
...
\subsection{1. Section/Function Title: Variable Initialization and Squareness Check}
\textbf{Explanation:} \\
...
\subsection{2. Section/Function Title: Reflexivity Check}
\textbf{Explanation:} \\
...
\subsection{3. Section/Function Title: Transitivity Check and Transitive Closure Computation}
\textbf{Explanation:} \\
...
\subsection{4. Section/Function Title: Symmetry Check and Equivalence Classification}
\textbf{Explanation:} \\
...
\subsection{5. Section/Function Title: Anti-Symmetry Check and Partial Order Condensation}
\textbf{Explanation:} \\
...
\textbf{Variable Outputs translated to Graphs:}
\begin{itemize}
    \item \textbf{Initial Adjacency Matrix ($R$):} Translates to a standard directed graph (digraph). The vertices (axes/nodes) represent the elements $1$ to $n_1$. An edge exists from node $i$ to $j$ if $R(i,j) = 1$.
