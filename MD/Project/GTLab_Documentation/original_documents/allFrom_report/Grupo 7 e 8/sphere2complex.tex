...
\chapter{MATLAB Code Analysis: \texttt{sphere2complex.m}}
\date{}
...
\subsection*{1. Section/Function Title: Stereographic Projection Mapping}
\textbf{2. Explanation:} \\
...
Note: If a point is exactly at the North Pole $v = (0, 0, 1)$, the denominator $(1 - z)$ evaluates to $0$. The numerators $x$ and $y$ are also $0$, resulting in $0/0$, which MATLAB evaluates to \texttt{NaN} (Not-a-Number), precisely as documented in the original file's comments.
...
\subsection*{1. Section/Function Title: Complex Variable Formation}
\textbf{2. Explanation:} \\
...
\begin{itemize}
    \item \textbf{Data Output:} The function returns a single MATLAB variable \texttt{w}, which is an $N \times 1$ column vector containing double-precision complex numbers (e.g., $2.5 + 3.1i$).
