...
\chapter{Análise do Script MATLAB: Discrete Math Anonymous Funções}
...
\begin{itemize}
    ...
    \item \textbf{Grafo Theory \& Permutation Groups:} Mapping integer partitions (signatures) to connected components and disjoint cycles.
\end{itemize}

\section{Análise Passo a Passo do Código}

\subsection{1. Título da Secção/Função: Matriz e Vetor Sizing Utilities}
...
\subsection{1. Título da Secção/Função: Divisores e Relação do Diagrama de Hasse}
...
\subsection{1. Título da Secção/Função: Propriedades de Relações Binárias}
...
\subsection{1. Título da Secção/Função: Endomapas e Bijeções Funcionais}
...
\subsection{1. Título da Secção/Função: Partições Inteiras}
...
\subsection{1. Título da Secção/Função: Signatures, Connected Components, and Cycles}
...
\begin{itemize}
    \item \textbf{Console Saída:} The script immediately outputs the string literal: \texttt{List of useful anonymous functions} to the command window.
    \item \textbf{The Relation Matriz ($R$):} The script quietly calculates a matrix $R$ for the divisors of 1000. The divisors of 1000 are: $1, 2, 4, 5, 8, 10, 20, 25, 40, 50, 100, 125, 200, 250, 500, 1000$ (16 divisors in total). 
    \begin{itemize}
