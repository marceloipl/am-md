...
\chapter{Análise do Código MATLAB: \texttt{sphere2plane.m}}
...
\subsection{1. Função Definition and Projection Logic}

\textbf{Explanation:}\\
This section defines the function signature and immediately applies the core mathematical projection logic. 
\begin{itemize}
    \item \textbf{General Objetivo:} To ingest 3D spatial coordinates and output their corresponding 2D mapped coordinates.
    \item \textbf{Entradas:} \texttt{x}, \texttt{y}, and \texttt{z} represent the Cartesian coordinates of points located on the surface of a 3D unit sphere.
    \item \textbf{Saídas:} \texttt{X} and \texttt{Y} represent the transformed Cartesian coordinates on the 2D projection plane.
...
\textbf{Console and Programmatic Saída:}\\
...
\begin{itemize}
    \item \textbf{Entrada Visualization (Axes $x, y, z$):} A 3D plot showing a hollow unit sphere centered at the origin $(0,0,0)$. 
    \item \textbf{Saída Visualization (Axes $X, Y$):} A flat 2D scatter or surface plot. 
